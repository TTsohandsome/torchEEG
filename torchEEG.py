...
# 循环处理被试数据
for i in range(1, 2):  # 原始MATLAB代码的49:50对应Python的range(49,51)
    if i < 10:
    ...
    for mat_file in mat_files:
        ...
        # 绘制训练历史
        grid.plot_training_history(example_history)
        
        # 绘制示例混淆矩阵
        y_true = labels.flatten()
        y_pred = np.random.choice([0, 1], size=len(y_true))  # 示例预测结果
        grid.plot_confusion_matrix(y_true, y_pred)
        
        # 绘制通道活动
        grid.plot_channel_activity()
        
        # 绘制地形图
        feature_values = np.random.randn(grid.channels)  # 示例特征值
        grid.plot_topographic_map(feature_values)
